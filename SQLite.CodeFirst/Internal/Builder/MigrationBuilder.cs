...
namespace SQLite.CodeFirst.Builder
{
    internal class MigrationBuilder
    {
        ...
        public IEnumerable<MigrationStatement> MigrationStatements => _migrationStatements;
        ...
        private void Generate(AddForeignKeyOperation op)
        {
            ...
            using (var tw = Format.CreateIndentedTextWriter())
            {
                ...
                var referenceList = op.DependentColumns.Select(dependentColumn =>
                    op.PrincipalColumns[op.DependentColumns.IndexOf(dependentColumn)]).ToList();
        ...
        [SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "op")]
        ...
        private void Generate(DropIndexOperation op)
        {
            ...
            {
                var indexName = op.HasDefaultName
                    ? string.Format(CultureInfo.InvariantCulture, "{0}_{1}", op.Name,
                        Format.ReservedWord(Format.RemoveDbo(op.Table)))
                    : op.Name;
        ...
        private void Generate(AddColumnOperation op)
        {
            ...
            using (var tw = Format.CreateIndentedTextWriter())
            {
                ...
                if (col.IsNullable != null
                    ...
                    && !col.IsTimestamp
                    && (
                        col.StoreType == null || !col.StoreType.Equals("rowversion", StringComparison.OrdinalIgnoreCase)
        ...
        [SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "opeMoveTable")]
        ...
        private bool Generate(ColumnModel column, IndentedTextWriter tw, PrimaryKeyOperation primaryKeyOp)
        {
            ...
            else
            {
                ...
                if (column.Annotations.TryGetValue("Unique", out var uniqueAnnotation))
                {
                    tw.Write(" UNIQUE" + Format.UniqueConflictText(uniqueAnnotation));
                }

                if (column.Annotations.TryGetValue("Collate", out var collateAnnotation))
                {
            ...
            foreach (var cmdTree in operation.CommandTrees)
            {
                ...
                if (cmd.Parameters.Count > 0)
                {
                    cmdText = _rxMatchParameterReference.Replace(
                        ...
                        m =>
                        {
                            var idx = int.Parse(m.Value.Substring(2), CultureInfo.InvariantCulture);
        ...
        [SuppressMessage("Microsoft.Usage", "CA1801:ReviewUnusedParameters", MessageId = "tableName")]
